...
for sequence in sequences:
	...
	GRAVY_scores.append(GRAVYscore)


print sorted(amino_acids)
print sorted(dipeptides_dict)

print GRAVY_scores[0:10]



#outputInfo = open(outputAnalysis, "w")

#outputInfo.writelines(str(sorted(amino_acids)) + "\n")
#outputInfo.writelines(str(sorted(dipeptides_dict)) + "\n")
#outputInfo.writelines("\n\n")

#for i in range(0,len(headers)):
#		outputInfo.writelines(headers[i])
#		outputInfo.writelines(str(aminoacid_frequencies[i]))
#		outputInfo.writelines("\n\n")
#		outputInfo.writelines(str(dipeptide_frequencies[i]))
#		outputInfo.writelines("\n\n")
...
GRAVYpoints = []
for i in range(0,len(GRAVY_scores)):
	if -25.0<GRAVY_scores[i] <12.0:
		GRAVYpoints.append((1, GRAVY_scores[i]))
	if -75.0<GRAVY_scores[i] <-25.1:
		GRAVYpoints.append((2, GRAVY_scores[i]))
	if -125.0<GRAVY_scores[i] <-75.1:
		GRAVYpoints.append((3, GRAVY_scores[i]))
	if -175.0<GRAVY_scores[i] <-125.1:
		GRAVYpoints.append((4, GRAVY_scores[i]))
	if -225.0<GRAVY_scores[i] <-175.1:
		GRAVYpoints.append((5, GRAVY_scores[i]))
	if -275.0<GRAVY_scores[i] <-225.1:
		GRAVYpoints.append((6, GRAVY_scores[i]))
	if -325.0<GRAVY_scores[i] <-275.1:
		GRAVYpoints.append((7, GRAVY_scores[i]))
fluClusters = KMeans(n_clusters = 3).fit(GRAVYpoints)
print fluClusters.cluster_centers_

x = []
y = []

for point in GRAVYpoints:
	x.append(point[0])
	y.append(point[1])
